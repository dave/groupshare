...
//go:generate sh -c "protoc --go_out=pb --go_opt=paths=source_relative --dart_out=../lib/pb --proto_path=../proto --proto_path=../../protod $(find ../proto/messages -iname '*.proto')"
...
// wrappers - to wrap database records - only used server-side so don't need dart output:
//go:generate sh -c "protoc --go_out=pb --go_opt=paths=source_relative --proto_path=../proto --proto_path=../../protod $(find ../proto/wrappers -iname '*.proto')"
