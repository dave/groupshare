...
func GetUserVerify(ctx context.Context, server *pserver.Server, tx *firestore.Transaction, token *auth_proto.Token) (*firestore.DocumentRef, *User, error) {
...
func LoginRequest(ctx context.Context, request, response *pmsg.Bundle) error {
	...
	response.MustSet(&auth_proto.Login_Response{Time: fmt.Sprint(t)})
...
func CodeRequest(ctx context.Context, server *pserver.Server, request, response *pmsg.Bundle) error {
	...
	response.MustSet(&auth_proto.Code_Response{Id: userRef.ID, Hash: hash})
	return nil
}
