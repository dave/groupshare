...
func TestProcessMessage(ctx context.Context, t *testing.T, server *pserver.Server, token *auth_proto.Token, message proto.Message) *pmsg.Bundle {
...
func ProcessMessage(ctx context.Context, server *pserver.Server, token *auth_proto.Token, message proto.Message, tasks bool) (*pmsg.Bundle, error) {
	request := pmsg.New()
	if token != nil {
...
func ProcessBundle(ctx context.Context, server *pserver.Server, request, response *pmsg.Bundle, tasks bool) (err error) {
	...
	if tasks {
		...
		case request.Has(&main_proto.Share_Delete_Task{}):
	...
	switch {
	case request.Has(&auth_proto.Login_Request{}):
		return auth.LoginRequest(ctx, request, response)
	case request.Has(&auth_proto.Code_Request{}):
		return auth.CodeRequest(ctx, server, request, response)
	}

	var user *auth.User
	token := &auth_proto.Token{}
	...
	ctx = context.WithValue(ctx, data_proto.UserContextKey, user)

	switch {
	case request.Has(&auth_proto.Validate_Request{}):
	...
	// other requests
	switch {
	case request.Has(&main_proto.Share_List_Request{}):
		return data.ShareListRequest(ctx, server, user, request, response)
	case request.Has(&main_proto.Share_Remove_Request{}):
		return data.ShareRemoveRequest(ctx, server, user, request, response)
	case request.Has(&main_proto.Share_Delete_Request{}):
