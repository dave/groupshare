...
func TestList(t *testing.T) {
	...
	c.MustRequest(ctx, t, token, false, &pstore.Payload_Edit_Request{
		...
		State:        0,
		Op:           pdelta.Root(&data.Share{Name: "b"}),
	}).MustGet(add)
	if add.State != 1 {
	...
	list := &data.Share_List_Response{}
	c.MustRequest(ctx, t, token, false, &data.Share_List_Request{}).MustGet(list)

	if !reflect.DeepEqual(list.Items, []*data.Share_List_Response_Item{{Id: string(id), Name: "b"}}) {
		t.Fatalf("expect %v, found %v", []*data.Share_List_Response_Item{{Id: string(id), Name: "b"}}, list.Items)
	}
...
func TestOps(t *testing.T) {
	...
	expected := "bCD"
	share := pdelta.MustUnmarshalAny(get.Value).(*data.Share)
...
func TestDeduplicationAdd(t *testing.T) {
	...
	c.MustRequest(ctx, t, token, false, &pstore.Payload_Edit_Request{
		...
		StateId:      string(pstore.NewStateID()),
		State:        0,
		Op:           pdelta.Root(&data.Share{Name: "c"}),
	}).MustGet(add2)
	...
	share := pdelta.MustUnmarshalAny(get1.Value).(*data.Share)
	if share.Name != "c" {
...
func mustUnmarshalOp(b []byte) *pdelta.Op {
	if len(b) == 0 {
		return nil
	}
	value := &pdelta.Op{}
...
func (c *Client) MustRequest(ctx context.Context, t *testing.T, token *auth.Token, tasks bool, request proto.Message) *pmsg.Bundle {
	t.Helper()
...
func (c *Client) Request(ctx context.Context, token *auth.Token, request proto.Message, tasks bool) (*pmsg.Bundle, error) {
	switch c.target {
	case LocalInProcess:
...
func (c *Client) localRequest(ctx context.Context, token *auth.Token, request proto.Message, tasks bool) (response *pmsg.Bundle, err error) {
